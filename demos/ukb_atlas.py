...
# Generate fiber orientations

# We combine all the outflow tracts into one entity, i.e the BASE
entities = [
...
bgrid.cell_data["Facet tags"] = ffun.values
bgrid.set_active_scalars("Facet tags")
p = pyvista.Plotter(window_size=[800, 800])
p.add_mesh(bgrid, show_edges=True)
if not pyvista.OFF_SCREEN:
...
values[:] = system.f0.x.array.reshape((geometry_f0.shape[0], 3))
