...
def test_refine_analytic_fibers(script, tmp_path: Path):
    ...
    script(outdir=outdir, create_fibers=True, comm=comm, ghost_mode=ghost_mode)
    geo = cg.Geometry.from_folder(comm=comm, folder=outdir, ghost_mode=ghost_mode)
...
def test_refine_biv(tmp_path: Path):
    ...
    outdir = path / "mesh"
    ghost_mode = dolfinx.mesh.GhostMode.none
    cg.mesh.biv_ellipsoid(outdir=outdir, create_fibers=True, comm=comm, ghost_mode=ghost_mode)
    geo = cg.Geometry.from_folder(comm=comm, folder=outdir, ghost_mode=ghost_mode)
    assert geo.f0 is not None
    assert (outdir / "mesh.xdmf").is_file()
    # assert geo.mesh.geometry.dim == 3
...
def test_refine_ukb(tmp_path: Path):
    ...
    outdir = path / "mesh"
    ghost_mode = dolfinx.mesh.GhostMode.none
    cg.mesh.ukb(outdir=outdir, create_fibers=True, comm=comm, ghost_mode=ghost_mode)
    geo = cg.Geometry.from_folder(comm=comm, folder=outdir, ghost_mode=ghost_mode)
    assert geo.f0 is not None
    assert (outdir / "mesh.xdmf").is_file()
    # assert geo.mesh.geometry.dim == 3
